...
def open_data(output_path: Path, redownload: bool):
    ...
    drugs = pd.read_csv(
        output_path / "drugrep/drugs.txt", sep="\t", skiprows=9, dtype=str
    )
    samples = pd.read_csv(
        output_path / "drugrep/samples.txt", sep="\t", skiprows=9, dtype=str
    )
...
def get_compound_annotations(output_dir: str, redownload: bool) -> pd.DataFrame:
    edges = open_data(Path(output_dir), redownload)
...
def broad_id_to_pubchem_mapper(output_dir, redownload: bool):
    edges = open_data(Path(output_dir), redownload)
