...
from jump_portrait.fetch import (build_s3_image_path, get_image_from_s3path,
...
@batch_processing
def save_image(
    row, channel: str, correction: str = "Orig", output_dir: str = "imgs", pad: int = 5
):
    s3_image_path = build_s3_image_path(row=row, channel=channel, correction=correction)
    image = get_image_from_s3path(s3_image_path)
