...
for dataset, filename in datasets_filenames:
    ...
    ncbi_formatter = (
        '{{"href": "https://www.ncbi.nlm.nih.gov/gene/{}", "label":"NCBI"}}'
    )
    url_template = (
    ...
    cycles = cycle(range(0 if dataset == "crispr" else 1, 9))
    jcp_ids = urls.select(pl.col(jcp_col)).to_series().to_numpy().astype("<U15")
    moving_idx = np.repeat(cycles, len(vals))
    ...
    ncbi_mapper = {
        jcp: ncbi_formatter.format(idx)
        for jcp, idx in zip(uniq_jcp, ncbi_mapper_values)
    }
    ...
    final_output = f"{dataset}.parquet"
